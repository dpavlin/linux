...
static inline bool
cmdstream_gart_bo(struct parser_state *ps,
		      ...
		      bool want_gather)
{
	...
	if (ps->opcode == HOST1X_OPCODE_GATHER) {
		DRM_ERROR("gather overwrites reloc\n");
		return -EINVAL;
	}
	...
	if (reloc_desc.bo_index >= ps->num_bos) {
		DRM_ERROR("invalid reloc bo index %u, num_bos %u\n",
	...
	if (is_gather != want_gather) {
		DRM_ERROR("invalid reloc bo type\n");
		return -EINVAL;
	}
	...
	if (offset >= max_size) {
		DRM_ERROR("invalid reloc bo offset %u, gem size %zu\n",
			  offset, max_size);
		return -EINVAL;
	}

	if (is_gather && (offset + ps->count * sizeof(u32) > max_size)) {
		DRM_ERROR("invalid gather size: offset %u, words %u, max size %zu\n",
...
non_incr:
	...
	case HOST1X_OPCODE_IMM:
		if (test_bit(ps->offset, ps->addr_regs)) {
			DRM_ERROR("writing immediate to address register\n");
			return -EINVAL;
		}
		break;
...
static inline int cmdstream_update_classid(struct parser_state *ps)
{
	...
	DRM_ERROR("invalid class id 0x%x\n", ps->classid);
...
cmdstream_patch_syncpt_incrs(struct parser_state *ps)
{
	...
	if (overflow) {
		DRM_ERROR("too many sync point increments\n");
		return -EINVAL;
	}
...
static inline int cmdstream_patch_extend(struct parser_state *ps)
{
	...
	default:
		DRM_ERROR("invalid extend subop %u\n", extend.subop);
		return -EINVAL;
	}

	if (err) {
		DRM_ERROR("invalid extend value %u\n", extend.value);
		return err;
	}
...
static inline int cmdstream_patch_host1x(struct parser_state *ps)
{
	...
	if (ps->count - ret != 0) {
		DRM_ERROR("writing to restricted register\n");
		return -EINVAL;
	}
...
static inline int cmdstream_patch(struct parser_state *ps)
{
	...
	if (!ps->classid) {
		DRM_ERROR("classid not selected\n");
		return -EINVAL;
	}
...
static inline int cmdstream_parse_opcode(struct parser_state *ps)
{
	...
	case HOST1X_OPCODE_GATHER:
		if (!(word & BIT(15))) {
			DRM_ERROR("only pure data-gather allowed\n");
			return -EINVAL;
		}

		ps->offset	= word >> 16 & 0xfff;
		ps->count	= word & 0x1fff;
		ps->mask	= 0;
	...
	case HOST1X_OPCODE_SETPYLD:
		DRM_ERROR("forbidden cdma opcode %08x\n", word);
		return -EINVAL;
	...
	case HOST1X_OPCODE_GATHER_W:
		/* unimplemented yet */
		DRM_ERROR("unsupported cdma opcode %08x\n", word);
		return -EINVAL;

	default:
		DRM_ERROR("invalid cdma opcode %08x\n", word);
		return -EINVAL;
	}

	if (ps->word_id + ps->count > ps->num_words) {
		DRM_ERROR("invalid number of words\n");
		return -EINVAL;
	}
