...
static u16 bs_ld_img_data(u8 *data, bool upd_area, u16 x, u16 y, u16 w, u16 h)
{
    ...
    else
    {
        ...
        // Determine the hardware slop factor for non-area updates (skipping the slop if we're
        // doing a direct load of the framebuffer).
        //
        if ( !upd_area )
            if ( (EINKFB_ORIENT_PORTRAIT == broadsheet_get_orientation()) && (BS97_INIT_VSIZE == bs_vsize) )
                if ( BS97_SOFT_VSIZE == w )
        ...
        for ( i = j = 0; i < data_size; )
        {
            ...
            if ( 0 == (++i % w) )
            {
                // Handle the hardware slop factor when necessary.
                //
                for ( k = 0; k < m; k++ )
                {
...
static u16 bs_cmd_orientation_to_rotmode(bool orientation)
{
...
void bs_cmd_print_disp_timings(void)
{
    if ( EINKFB_DEBUG() )
    {
        ...
        einkfb_print_info("disp_timings: vsize=%d vsync=%d vblen=%d velen=%d\n", vsize, vsync, vblen, velen);
        einkfb_print_info("disp_timings: hsize=%d hsync=%d hblen=%d helen=%d\n", hsize, hsync, hblen, helen);
    }
}
...
#define PROMOTE_FLASHING_UPDATE()                           \
    if ( broadsheet_promote_flashing_updates() )            \
        bs_upd_mode = BS_UPD_MODE(BS_UPD_MODE_GC);          \
    else                                                    \
    {                                                       \
        if ( BS_UPD_MODE_MU == BS_UPD_MODE(bs_upd_mode) )   \
            bs_upd_mode = BS_UPD_MODE(BS_UPD_MODE_GU);      \
    }
...
static void bs_cmd_ld_img_upd_data_which(bs_cmd cmd, fx_type update_mode, u8 *data, u16 x, u16 y, u16 w, u16 h)
{
    if ( data )
    {
        bool    upd_area = (bs_cmd_LD_IMG_AREA == cmd) || UPD_MODE_INIT(bs_upd_mode),
        ...
        if ( upd_area )
        {
            if ( UPDATE_AREA_FULL(update_mode) )
            {
                // Promote the flashing update if we should.
                //
                PROMOTE_FLASHING_UPDATE();
            }
            else
            {
        ...
        // Save how long it took to perform various image-related operations.
        //
...
static void bs_get_resolution(unsigned char fpl_size, bool orientation, u32 *xres_hw, u32 *xres_sw, u32 *yres)
{
    switch ( fpl_size )
    {
        ...
        case EINK_FPL_SIZE_97:
            *xres_hw = (EINKFB_ORIENT_PORTRAIT == orientation) ? BS97_HARD_VSIZE : BS97_INIT_HSIZE;
            *xres_sw = (EINKFB_ORIENT_PORTRAIT == orientation) ? BS97_SOFT_VSIZE : BS97_INIT_HSIZE;

            *yres    = (EINKFB_ORIENT_PORTRAIT == orientation) ? BS97_INIT_HSIZE : BS97_SOFT_VSIZE;
        break;
        ...
        default:
            *xres_hw = (EINKFB_ORIENT_PORTRAIT == orientation) ? BS60_INIT_VSIZE : BS60_INIT_HSIZE;
            *xres_sw = *xres_hw;
            
            *yres    = (EINKFB_ORIENT_PORTRAIT == orientation) ? BS60_INIT_HSIZE : BS60_INIT_VSIZE;
        break;
    }
}

#define BS_SW_INIT_CONTROLLER(f) bs_sw_init_controller(f, EINKFB_ORIENT_PORTRAIT, NULL, NULL, NULL)

void bs_sw_init_controller(bool full, bool orientation, u32 *xres_hw, u32 *xres_sw, u32 *yres)
{
    ...
    if ( fpl_size )
        bs_get_resolution(fpl_size, orientation, xres_hw, xres_sw, yres);
}
