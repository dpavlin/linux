...
static u16 bs_cmd_orientation_to_rotmode(bool orientation, bool upside_down)
{
    ...
    // Displays connected to the ADS & Mario boards are generally upside down with respect
    // to everything else.  So, "fix" that here.
    //
    if ( IS_ADS() || IS_MARIO() )
    {
        switch ( result )
...
void bs60_init(int wa, bool full)
{
    ...
    if ( BS_ISIS() )
    {
        ...
        bs60_init_lutidxfmt |= (BS60_INIT_AUTO_WF | BS60_INIT_PIX_INVRT);
    }
...
void bs97_init(int wa, bool full)
{
     ...
     if ( BS_ISIS() )
    {
        ...
        bs97_init_lutidxfmt |= (BS97_AUTO_WF | BS97_INIT_PIX_INVRT);
    }
