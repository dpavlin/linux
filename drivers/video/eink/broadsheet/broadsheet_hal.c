...
bool broadsheet_get_orientation(void)
{
    ...
    return ( ORIENTATION(info.xres, info.yres) );
}

bool broadsheet_promote_flashing_updates(void)
{
    struct einkfb_info info; einkfb_get_info(&info);
    
    return ( EINKFB_RESTORE(info) || promote_flashing_updates );
}
...
static bool broadsheet_sw_init(struct fb_var_screeninfo *var, struct fb_fix_screeninfo *fix)
{
    ...
    //
    bs_sw_init_controller(FULL_BRINGUP_CONTROLLER, bs_orientation, &xres_hw, &xres_sw, &yres);
    scratchfb_size = BPP_SIZE((xres_hw*yres), EINKFB_8BPP);
    ...
    if ( scratchfb )
    {
        switch ( bs_bpp )
        {
            ...
            case EINKFB_2BPP:
            case EINKFB_4BPP:
                broadsheet_size                 = BPP_SIZE((xres_sw*yres), bs_bpp);
        ...
        broadsheet_var.yres = broadsheet_var.yres_virtual = yres;
