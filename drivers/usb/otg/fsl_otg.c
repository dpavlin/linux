...
/* PM timer */
struct timer_list	otg_pm_timer;
static int hcd_resume_ready;
...
void otg_pm_timer_func(unsigned long data)
{
	struct otg_transceiver *otg = &fsl_otg_dev->otg;
	struct otg_fsm *fsm = &fsl_otg_dev->fsm;
	struct usb_hcd *hcd;
	u32 otg_sc;

	hcd = container_of(otg->host, struct usb_hcd, self);
	if (hcd_resume_ready == 0) {
		if (hcd->state == HC_STATE_RUNNING)
			hcd_resume_ready = 1;
		mod_timer (&otg_pm_timer, jiffies + msecs_to_jiffies(300));
		return;
	}
	...
	otg_sc = le32_to_cpu(usb_dr_regs->otgsc);
	otg_sc |= OTGSC_INTSTS_MASK | OTGSC_IE_USB_ID;
	usb_dr_regs->otgsc = cpu_to_le32((le32_to_cpu(usb_dr_regs->otgsc) |
  				 OTGSC_INTSTS_MASK | OTGSC_IE_USB_ID));
...
int usb_otg_start(struct platform_device *pdev)
{
	...
	init_timer(&otg_pm_timer);
	otg_pm_timer.function = otg_pm_timer_func;
...
static ssize_t
show_linestatus(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 portsc_val = le32_to_cpu(usb_dr_regs->portsc);
	int d_plus, d_minus;

	d_plus = ((portsc_val & USB_PORTSC_D_PLUS_STATUS_MASK) != 0);
	d_minus = ((portsc_val & USB_PORTSC_D_MINUS_STATUS_MASK) != 0);

	return sprintf(buf, "%d%d\n", d_plus, d_minus);
}

static DEVICE_ATTR(linestatus, S_IRUGO, show_linestatus, NULL);

static int add_sysfs_entries(struct device *dev)
{
	return sysfs_create_file(&dev->kobj, &dev_attr_linestatus.attr);
}

static void remove_sysfs_entries(struct device *dev)
{
	sysfs_remove_file(&dev->kobj, &dev_attr_linestatus.attr);
}
...
static int fsl_otg_suspend(struct platform_device *pdev, pm_message_t state)
{
	u32 temp;
	...
	temp = readl(&fsl_otg_dev->dr_mem_map->otgsc);
	temp &= ~OTGSC_IE_USB_ID;
	writel(temp, &fsl_otg_dev->dr_mem_map->otgsc);
	hcd_resume_ready = 0;
	
	if (fsl_otg_dev->fsm.id){
		fsl_otg_start_gadget(&fsl_otg_dev->fsm, 0);
		otg_drv_vbus(&fsl_otg_dev->fsm, 1);
		fsl_otg_start_host(&fsl_otg_dev->fsm, 1);
	}
