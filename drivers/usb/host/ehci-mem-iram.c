...
{
	if (g_usb_pool_count) {
		pr_debug(KERN_INFO "usb_pool_initialize : already initialzed.\n");
		return 0;
	}

	pr_debug ("Initializing the USB Static IRAM pool - 4096/16\n");
...
static void use_buffer(struct ehci_hcd *ehci, int address)
{
	...
	if (ehci->usb_address[0] == 0) {
		...
		pr_debug(KERN_INFO "usb_address[0] %x\n", address);
		return;
	} else if (ehci->usb_address[1] == 0) {
		ehci->usb_address[1] = address;
		pr_debug(KERN_INFO "usb_address[1] %x\n", address);
		return;
	} else
...
static struct ehci_qh *ehci_qh_alloc(struct ehci_hcd *ehci, gfp_t flags)
{
	...
	++g_debug_qH_allocated;
	if (qh == NULL) {
		pr_debug("run out of i-ram for qH allocation\n");
		return qh;
	}
