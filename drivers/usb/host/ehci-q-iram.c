	...
	if (token & QTD_STS_HALT) {
		if (token & QTD_STS_BABBLE) {
			...
			if (QTD_CERR(token))
				...
				pr_debug("devpath %s ep%d%s 3strikes\n",
					 urb->dev->devpath,
			       ...
			       struct urb *urb, gfp_t flags)
{
	...
	case USB_SPEED_HIGH:	/* no TT involved */
		...
		break;
	default:
		pr_debug("bogus dev %p speed %d", urb->dev, urb->dev->speed);
done:
		qh_put(qh);
		return NULL;
	}
				     ...
				     int epnum, void **ptr)
{
	...
	u32 qh_addr_mask = cpu_to_le32(0x7f);
...
static int
submit_async(struct ehci_hcd *ehci, struct usb_host_endpoint *ep,
	     struct urb *urb, struct list_head *qtd_list, gfp_t mem_flags)
{
	...
	pr_debug(
		 ...
		 urb->transfer_buffer_length, qtd, ep->hcpriv);
