...
void pmic_get_revision(pmic_version_t * ver)
{
	...
	/*
	 * Turn off the MMC1 and MMC2 regulators. The MMC probe
	 * function should turn them back on using the set_ios()
	 * call. This is done so that the EXT_CSD can be read 
	 * correctly.
	 */
	...
	/*
	 * power button - system resets if button held for 4 seconds
	 */
	pmic_power_set_auto_reset_en(1);
	pmic_power_set_conf_button(BT_ON1B, 1, 4);
...
unsigned int pmic_get_active_events(unsigned int *active_events)
{
	...
	if (status1 & (1 << 3))
		pmic_write_reg(REG_INTERRUPT_MASK_1, (1 << 3), (1 << 3));
