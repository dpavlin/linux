...
#if 0
#define adc_debug(fmt, args...) printk(KERN_ERR "adc: " fmt, ##args)
/* FIXME */
#else
#define adc_debug(fmt, args...) pr_debug(fmt, ##args)
#endif
...
PMIC_STATUS mc13783_adc_convert(t_adc_param * adc_param)
{
	...
	if (use_bis == 0) {
		...
		pmic_write_reg(REG_INTERRUPT_STATUS_0, (1 << 0), (1 << 0));
		data_ready_adc_1 = false;
	} else {
		data_ready_adc_2 = false;
		adc_1_reg |= ASC_ADC;
		...
		data_ready_adc_2 = false;
		pmic_write_reg(REG_INTERRUPT_STATUS_0, (1 << 1), (1 << 1));
	}
...
/*
 * The two functions that call this routine have translated the incoming
 * caller's channel number into the correct MC13783 ADC channel number
 * before calling this routine.  However, the switch statement uses the
 * original channel numbers, not the actual channel numbers.
 *
 * For example, LICELL is defined in an enumeration as 18, but the actual
 * channel number passed to this routine is channel_num[18] - which is 6,
 * which is correct.
 *
 * I don't know why they remap the channel numbers, but this fix will cause
 * the proper bitmask to be returned.
 */
	...
	case 6: /* LICELL: */
		...
		break;
	case 4: /* CHARGE_CURRENT: */
		read_mode = M_CHARGE_CURRENT;
		break;
	case 1: /* BATTERY_CURRENT: */
		read_mode = M_BATTERY_CURRENT;
		break;
	case 5: /* THERMISTOR: */
		read_mode = M_THERMISTOR;
		break;
	case 7: /* DIE_TEMP: or USB_ID: */
		/* These two values are read using the same channel, but
		 * need to enable different ADC bits - probably need to
		 * be mutually exclusive, but fixing them would require
		 * reworking more code.
		 * If this is a problem, we can revisit this.
		 */
		read_mode = M_DIE_TEMPERATURE | M_UID;
		break;
	default:
		read_mode = 0;
	}
adc_debug("%s(channel=%d) returns 0x%x\n", __FUNCTION__, channel, read_mode);
	return read_mode;
#else
	switch (channel) {
	case LICELL:
...
adc_debug("pmic_adc_convert(channel=%d)\n", channel);
	if (suspend_flag == 1) {
		return -EBUSY;
	}

adc_debug("channel (before) = %d\n", channel);
	channel = channel_num[channel];
adc_debug("channel (after)  = %d\n", channel);
	if (channel == -1) {
		adc_debug("Wrong channel ID\n");
...
adc_debug("pmic_adc_convert: result = 0x%04x\n", *result);
	return ret;
}
