 ...
 * Modifications Copyright 2008 Lab126.  All Rights reserved.
 */
...
#define KB_REV_DEFAULT 0
#define KB_REV_NELL_EVT1 4
#define KB_REV_NELL 5
static int kb_rev = KB_REV_DEFAULT;
module_param(kb_rev, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(kb_rev, "Keyboard revision; 0=Fiona keypad, 1=Turing with trackball, 2=Turing with rev0 fiveway, 3=Turing with EVT1 fiveway, 4=Nell EVT1, 5- Nell EVT2+ (default is 0)");
...
void report_event(struct input_dev *dev, unsigned int type, unsigned int code, int value )
{
	...
	if (value == 1) {
	   ...
	   if( kobject_uevent( &mxckbd_dev->cdev.kobj, KOBJ_CHANGE ) )
	   {
...
static int mxc_kpp_probe(struct platform_device *pdev)
{
	...
	mxckpd_keycodes += (mxckpd_keycodes_size * kb_rev);
...
static int mxc_kpp_remove(struct platform_device *pdev)
{
	...
	remove_proc_entry(KEYPAD_PROC_FILE, &proc_root);
