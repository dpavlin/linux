...
static void _nfc_clk_disable(struct clk *clk)
{
	u32 reg;

	reg = __raw_readl(clk->enable_reg);
	reg |= (7 << clk->enable_shift);
	__raw_writel(reg, clk->enable_reg);
}
...
static struct clk nfc_clk = {
	...
	.enable = _clk_enable,
	.enable_reg = MXC_CCM_PDR0,
	.enable_shift = MXC_CCM_PDR0_NFC_PODF_OFFSET,
	.disable = _nfc_clk_disable,
};
...
int __init mxc_clocks_init(void)
{
	...
	/* set_rate(usb pll,240Mhz) */
	clk_set_rate(&usb_pll_clk, 240000000);
...
unsigned long __init clk_early_get_timer_rate(void)
{
	...
	/*
	 * the time init code calls this function really early. Right time 
	 * to put the configure the clk rate to 532 MHz
	 */
	__raw_writel(0xFF871D58,MXC_CCM_PDR0);
	__raw_writel(0x000c2803,MXC_CCM_MPCTL);
	_clk_pll_set_rate(&mcu_pll_clk, 532000000);
