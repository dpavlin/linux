...
int audio_playing_flag = 0;
EXPORT_SYMBOL(audio_playing_flag);
...
static void mxc_sdma_channel_work(struct work_struct *work)
{
	dma_request_t request_t;
	dma_channel_params chnl_param;
	mxc_dma_channel_t *chnl_info;
	mxc_dma_channel_private_t *data_priv = 
				container_of(work, mxc_dma_channel_private_t, sdma_channel_work);
	int bd_intr = 0, error = MXC_DMA_DONE;
	int arg = data_priv->channel_num;
...
int mxc_dma_request(mxc_dma_device_t channel_id, char *dev_name)
{
	...
	if (ret == 0) {
		...
		data_priv->channel_num = channel_num;
		INIT_WORK(&data_priv->sdma_channel_work, mxc_sdma_channel_work);
...
EXPORT_SYMBOL(audio_channel_num);
...
static void mxc_dma_chnl_callback_audio(void *arg)
{
	...
	for (i=0; i<64; i++)
		if (mxc_sdma_channeltasklet_audio(priv) == 0)
			break;
}
		   ...
		   int num_buf, mxc_dma_mode_t mode)
{
	...
	if ((chnl_param.peripheral_type != MEMORY) && (mode != chnl_info->mode)) {
		...
		if (channel_num == audio_channel_num)	
			chnl_param.callback = mxc_dma_chnl_callback_audio;
		else
...
int mxc_dma_callback_set_audio(int channel_num,
			 mxc_dma_callback_t callback, void *arg)
{
	if ((channel_num >= MAX_DMA_CHANNELS) || (channel_num < 0)) {
		return -EINVAL;
	...
	mxc_dma_set_callback(channel_num, mxc_dma_chnl_callback_audio,
			     (void *)channel_num);

	return 0;
}

EXPORT_SYMBOL(mxc_dma_callback_set_audio);
...
int mxc_dma_enable(int channel_num)
{
 ...
 /*!
 * This handles the pause/ playback requests from the Audio driver
 *
 * @param channel_num  the channel number returned at request time. This
 *                     would be used by the DMA driver to identify the calling
 *                     driver and do the necessary cleanup on the channel
...
int mxc_dma_pause(int channel_num){
	mxc_dma_channel_t *chnl_info;
	mxc_dma_channel_private_t *data_priv;	
	...
	chnl_info->curr_buf = data_priv->buf_tail;
	return 0;
}	
